...
def crop_ai_icons_perfect():
    ...
    # Exact mathematical centers of the 4 rows x 5 columns grid
    # Spacing is perfectly 197px horizontal, ~257px vertical
    x_coords = [118, 315, 512, 709, 903]
    y_coords = [131, 388, 653, 903]
    
    circles_sorted = []
    for y in y_coords:
        for x in x_coords:
            circles_sorted.append((x, y, 86))

    print(f"Constructed {len(circles_sorted)} perfect mathematical grid centers.")

    # 1-to-1 Mapping coordinates from the perfect grid:
    ...
    for name, idx in mappings.items():
        ...
        left = max(0, cx - pad)
        top = max(0, cy - pad)
        right = min(pil_img.size[0], cx + pad)
        bottom = min(pil_img.size[1], cy + pad)
